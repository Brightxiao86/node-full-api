...
var cartModel = require('../model/weakbook/cartModel')
var orderModel = require('../model/weakbook/orderModel');
...
router.get('/delGood', function(req, res, next) {
  ...
  GoodNewly.deleteOne({_id:id}).then(()=>{
...
router.get('/getHotGoodList', function(req, res, next) {
  ...
  GoodNewly.find().then(arr=>{
    let total = arr.length
    GoodNewly.find(params).limit(size).skip((page-1)*size).sort({create_time: -1}).then(list=>{
...
router.get('/getGoodDetail', function(req, res, next) {
  ...
  GoodNewly.find({_id:good_id}).then(arr=>{
  ...
  jwt.verifyToken(req, res).then(user=>{
    userModel.find(user).then((userArr)=>{
      ...
      cartModel.find({status:1, user_id: userArr[0]._id}).limit(size).skip((page-1)*size).sort({create_time: -1}).then(arr1=>{
        ...
        // 遍历获取商品信息，一起传递给购物车列表
        arr1.map((ele,idx)=>{
          GoodNewly.find({_id: ele.good_id}).then(arr2=>{
            list.push({
              _id: ele._id,
              good_id: ele.good_id,
